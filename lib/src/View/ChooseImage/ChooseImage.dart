...
import '../../Widget/LineBaseWidget.dart';
...
class ChooseImage {
  ...
  final Function? onActionTakePicture;
  final Function? onHideLoading;
  final VoidOnChooseImage? onChooseImage;
  final Function(List<File> file)? onChooseMultiImage;

  final BuildContext context;
  final bool isCrop;
  final bool isMultiImage;
  final String? title;
  final Color colorTitle, colorClose, colorTypeCamera, colorTypeGallery;
  ...
  ChooseImage({
    ...
    this.onHideLoading,
    this.onShowLoading,
    this.colorTitle = const Color(0xFF14142B),
    this.colorClose = const Color(0xFF14142B),
    this.colorTypeCamera = const Color(0xff007AFF),
    this.colorTypeGallery = const Color(0xff007AFF),
  });
  ...
  int _getFirstVersionOS() {
    String osVersion = Platform.operatingSystemVersion;
    return int.parse(osVersion.split(".").first);
  }
  ...
  _chooseImage(ImageSource imageSource) async {
    ...
    if (isMultiImage && imageSource == ImageSource.gallery && Platform.isIOS && _getFirstVersionOS() < 14) {
      XFile? xFile;
      try {
...
//    );
    await Future.delayed(const Duration(milliseconds: 100));
    debugPrint("_file?.path: ${_file?.path}");
    final file = await ImageCrop.cropImage(
      file: _file!,
      area: area,
      scale: 1,
    );
    debugPrint("file.path: ${file.path}${file.path}");
